        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['adabound']:
                    ...
                    if state['step'] == 1 or state['step'] % 10000 == 0:
                        torch.save([step_size.div(denom), lower_bound, upper_bound], 'adabound_ite%d_g{}_flr{}.pt'.format(group['gamma'], group['final_lr'])%state['step'])
                    step_size.div_(denom).clamp_(lower_bound, upper_bound).mul_(exp_avg)
                    p_data_fp32.add_(-step_size)
                else:
                    if state['step'] == 1 or state['step'] % 10000 == 0:
                        if amsgrad:
                            torch.save(step_size/denom, 'amsgrad_ite%d_lr%.2e.pt' % (state['step'], group['base_lr']))
                        else:
                            torch.save(step_size / denom, 'adam_ite%d_lr%.2e.pt' % (state['step'], group['base_lr']))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if state['step'] == 1 or state['step'] % 10000 == 0:
                    torch.save(step_size / denom, 'madam_ite%d.pt'%state['step'])
