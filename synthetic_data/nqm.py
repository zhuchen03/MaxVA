...
if __name__ == "__main__":
    ...
    lr_adam_list = [1e-3, 2e-3, 3e-3, 4e-3, 5e-3, 6e-3, 7e-3, 8e-3, 9e-3, 1e-2,
                    2e-2, 3e-2, 4e-2, 5e-2, 6e-2, 7e-2, 8e-2, 9e-2, 1e-1, 2e-1]
    lr_madam_list = lr_adam_list
    n_iters = 1000
    ...
    # plot the errors and standard deviations
    adam_min_err, madam_min_err = float('inf'), float('inf')
    for key, val in adam_errs.items():
        adam_lr = key.split('_')[0][2:]
        std_err = np.std(val) / math.sqrt(n_trials)
        err = np.mean(val)

        if err < adam_min_err:
            adam_min_err = err
            adam_min_stder = std_err
            adam_min_lr = adam_lr
