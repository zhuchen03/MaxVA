...
def plot_curves(opt_dict_dict, plot_key, ylabel, downsample=10, set_ylim_min=False, ymax=1e2):
    ...
    for key in keys:
        ...
        plt.plot(np.arange(len(data_list))+1, data_list, color=color_dict[key], label=key, ls=line_style_dict[key],
                 alpha=0.9)
    plt.legend()
    ...
    ax.set_yscale("log")
    if set_ylim_min:
        ax.set_ylim(bottom=1e-8)#, ymax=ymax)
    # else:
    #     ax.set_ylim(ymax=ymax)
    plt.savefig("plots/nonconvex-{}.pdf".format(plot_key), bbox_inches='tight')
...
if __name__ == "__main__":
    # lr = 1 #* 1.5
    resume = "" #"#"results/nonconvex_results.json"
    if resume:
        opt_dict_dict = json.load(open(resume))
        plot_curves(opt_dict_dict, "total_alr", r"$\sum_{t=1}^{T} 1/\sqrt{v_t}$", ymax=1e4)
        plot_curves(opt_dict_dict, "total_astep", r"$\sum_{t=1}^{T}|| g_t/\sqrt{v_t}||^2$", ymax=5e2)
        plot_curves(opt_dict_dict, "total_clr",
                    r"$\sum_{t=1}^{T}|| \frac{\eta_{t}}{\sqrt{v_t}}-\frac{\eta_{t-1}}{\sqrt{v_{t-1}}} ||_1$", ymax=1e1)
        plot_curves(opt_dict_dict, "grad", r"$|g_t|$")
        plot_curves(opt_dict_dict, "x", r"$f(x)$", set_ylim_min=True, ymax=1)
    else:
        b_adam = 0.9
        ...
        plot_curves(opt_dict_dict, "total_clr", r"$\sum_{t=1}^{T}|| \frac{\eta_{t}}{\sqrt{v_t}}-\frac{\eta_{t-1}}{\sqrt{v_{t-1}}} ||_1$")
        plot_curves(opt_dict_dict, "grad", r"$|g_t|$")
        plot_curves(opt_dict_dict, "x", r"$f(x)$", set_ylim_min=True)
        json.dump(opt_dict_dict, open("results/nonconvex_results.json", 'w'))
